...
async def get_movie_sources(imdb_id: Optional[str], title: str, year: str) -> dict:
    ...
    for provider in YTS_PROVIDERS:
        try:
            logger.info(f"Movie lookup provider={provider} imdb={imdb_id} title={title}")
            data = await fetch_json(f"{provider}/list_movies.json", {"query_term": query, "limit": 5})
            movies = data.get("data", {}).get("movies") or []

            if not movies:
                data = await fetch_json(f"{provider}/list_movies.json", {"query_term": title, "limit": 5})
                movies = data.get("data", {}).get("movies") or []

            if not movies:
                result["providers"][provider] = {"success": False, "error": "No movie match"}
                continue
            ...
            for torrent in movie.get("torrents", []):
                ...
                result["sources"].append({
                    ...
                    "hash": hash_,
                    "magnet": build_magnet(hash_, f"{movie.get('title', title)} {quality}"),
            ...
            result["providers"][provider] = {"success": True, "sources": len(result["sources"])}

            if result["found"]:
                result["sources"].sort(key=lambda x: QUALITY_ORDER.get(x["quality"], 99))
                return result
        except Exception:
            logger.exception(f"Movie provider failed: {provider} imdb={imdb_id} title={title}")
            result["providers"][provider] = {"success": False, "error": "Provider request failed"}
...
async def get_episode_sources(imdb_id: str, season: int, episode: int, title: str) -> dict:
    ...
    for provider in EZTV_PROVIDERS:
        try:
            logger.info(f"TV lookup provider={provider} imdb={imdb_numeric} {ep_tag}")
            ...
            for t in torrents:
                ...
                matches = ((str(t.get("season")) == str(season) and str(t.get("episode")) == str(episode)) or ep_tag.lower() in filename.lower())
                ...
                fn = filename.upper()
                if "2160P" in fn or "4K" in fn: quality = "2160p"
                ...
                result["sources"].append({
                    "quality": quality,
                    "filename": filename,
                    "size": fmt_size(int(t.get("size_bytes") or 0)),
                    "size_bytes": int(t.get("size_bytes") or 0),
                    "seeds": t.get("seeds", 0),
                    "peers": t.get("peers", 0),
                    "hash": hash_,
                    "magnet": t.get("magnet_url") or build_magnet(hash_, filename),
            ...
            if result["found"]:
                result["sources"].sort(key=lambda x: (QUALITY_ORDER.get(x["quality"], 99), -x["seeds"]))
                return result
        except Exception:
            logger.exception(f"TV provider failed: {provider} imdb={imdb_id} {ep_tag}")
            result["providers"][provider] = {"success": False, "error": "Provider request failed"}
